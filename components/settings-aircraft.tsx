...
export function SettingsAircraft() {
  ...
  const fetchAircraft = async () => {
    try {
      ...
      // Extract the planes array from the response - handle new nested structure
      const planesData = data.data?.planes || data.planes || []
  ...
  const handleEditAircraft = async () => {
    ...
    if (editingAircraft && editingAircraft.registration && editingAircraft.type && editingAircraft.aircraftModel) {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/organizations/${organizationId}/planes/${editingAircraft.id}`,
          {
            method: "PUT",
            headers: {
              "Accept": "application/json",
  ...
  const handleDeleteAircraft = async () => {
    ...
    if (deleteAircraftId) {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/organizations/${organizationId}/planes/${deleteAircraftId}`,
          {
            method: "DELETE",
            headers: {
              "Accept": "application/json",
